...
% v - variant of picking the active samples
    ...
    while size(X,1)>0
        if (strcmpi(sampling_type,'RIPR'))
            ...
            RIPRactModel = RIPRsscactUpdateModel(RIPRactModel,X(idx_auto,:),Y(idx_auto,:),X(~idx_auto,:),lambda);
            ...
            %idx_correct = (RIPRclsTestResults.Yhat == Y);
            RIPRactModel = RIPRsscactUpdateModel(RIPRactModel,X(idx_manual,:),Y(idx_manual,:),X(~idx_manual,:),lambda);
            k = k+1; RIPRactModelTrace{k} = RIPRactModel;
            ...
            if(use_correct_only)
                idx_correct = (RIPRclsTestResults.Yhat == Y);
                idx_use = idx_active & idx_correct;
            else
                idx_use = idx_active;
            end
            save('debug_active_ripr.mat');
            RIPRactModel = RIPRsscactUpdateModel(RIPRactModel,X(idx_use,:),Y(idx_use,:),X(~idx_active,:),lambda);
            k = k+1; RIPRactModelTrace{k} = RIPRactModel;
...
function [RIPRclsModel] = RIPRsscclsTrainModel(X,Y,Xu,d,lambda)
    X = [X;Xu];
    Y = [Y;-ones(size(Xu,1),1)];
    RIPRclsModel = RIPRclsTrainModel(X,Y,d,lambda);
end

function [RIPRactModel] = RIPRsscactUpdateModel(RIPRactModel, X, Y, Xu, lambda)
    [RIPRactModel] = RIPRsscclsTrainModel([RIPRactModel.X;X],[RIPRactModel.Y;Y],Xu,RIPRactModel.d,lambda);
end
...
function idx_info_gain = PickInfoGainSamples(RIPRactModel, RIPRclsTestResults, X, r, d)
    if (r > size(X,1))
        idx_info_gain = true(size(X,1),1);
    else
        [sorted, idx] = sort(ComputeInfoGain(RIPRactModel.X, RIPRactModel.Y, X, d),'descend');
        idx_info_gain = false(size(X,1),1);
        idx_info_gain(idx(1:r)) = true;
    end
end
