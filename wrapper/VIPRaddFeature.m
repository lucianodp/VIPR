function [VIPRmodel] = VIPRaddFeature(VIPRmodel, s, featureid, featurename, ComputeLoss, LossParams, Solver)
...
% This function adds the specified feature to the specified submodel s.
    if(s <= length(VIPRmodel.Submodel))
        ...
        proj_new = [VIPRmodel.Submodel{s}.ProjDim, featureid];
        ...
        if (isfield(VIPRmodel.Submodel{s},'FeatureNames'))
            Submodel.FeatureNames = [VIPRmodel.Submodel{s}.FeatureNames, featurename];
        end
